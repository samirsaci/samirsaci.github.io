...
This chart animates COVID-19 cases for the top 15 countries since late January 2020 on a daily basis. Color indicates continents and data was sourced from [Johns Hopkins University](https://github.com/CSSEGISandData/COVID-19). Inspired by https://observablehq.com/@d3/bar-chart-race `
  ...
  main.variable(observer("countryframes")).define("countryframes", ["d3", "keyframes"], function (d3, keyframes) {
    return (
      d3.groups(keyframes.flatMap(([, data]) => data), d => d.country)
    )
  });
  main.variable(observer("prev")).define("prev", ["countryframes", "d3"], function (countryframes, d3) {
    return (
      new Map(countryframes.flatMap(([, data]) => d3.pairs(data, (a, b) => [b, a])))
    )
  });
  main.variable(observer("next")).define("next", ["countryframes", "d3"], function (countryframes, d3) {
    return (
  ...
  main.variable(observer("axis")).define("axis", ["margin", "d3", "x", "barSize", "n", "y"], function (margin, d3, x, barSize, n, y) {
    return (
      function axis(svg) {
  ...
  main.variable(observer()).define(["datevalues"], function (datevalues) {
    return (
      datevalues[40]
    )
  });
